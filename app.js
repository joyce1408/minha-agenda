const state = {
  ...
  filter: 'all',
  showSource: true,
  events: JSON.parse(localStorage.getItem('ma-v2-events') || 'null') || [
  ...
  tasks: JSON.parse(localStorage.getItem('ma-v2-tasks') || 'null') || [
  ...
  reminders: JSON.parse(localStorage.getItem('ma-v2-reminders') || 'null') || [
...
function save(){ localStorage.setItem('ma-v2-events',JSON.stringify(state.events)); localStorage.setItem('ma-v2-tasks',JSON.stringify(state.tasks)); localStorage.setItem('ma-v2-reminders',JSON.stringify(state.reminders)); }

function eventsFor(date){const key=isoDate(date);return state.events.filter(e=>e.date===key).sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99'))}
...
function render(){
  ...
  $('#summaryUrgent').textContent = state.events.filter(e=>{const d=new Date(e.date+'T00:00:00'); const now=new Date(); const diff=(d-new Date(now.getFullYear(),now.getMonth(),now.getDate()))/86400000; return diff>=0&&diff<=3&&!e.done}).length;
  renderCalendar();
  renderUpcoming();
  renderTasks();
  renderReminders();
...
function renderDay(){
  const items=eventsFor(state.date);
...
function eventChip(e){return `<div class="event-chip ${sourceClass(e.source)} ${e.done?'done':''}"><strong>${e.time||'Sem horário'} · ${escapeHtml(e.title)}</strong><small>${e.location?escapeHtml(e.location)+' · ':''}${sourceLabel[e.source]}</small></div>`}

function startOfWeek(d){const x=new Date(d); const day=x.getDay(); x.setDate(x.getDate()-(day===0?6:day-1)); return x}
function renderWeek(){
  const start=startOfWeek(state.date), times=Array.from({length:14},(_,i)=>i+8);
  const days=Array.from({length:7},(_,i)=>addDays(start,i));
  return `<div class="week-grid"><div class="week-header"><div></div>${days.map(d=>`<div>${new Intl.DateTimeFormat('pt-BR',{weekday:'short'}).format(d)}<br><b>${pad(d.getDate())}</b></div>`).join('')}</div><div class="week-body">${times.map(h=>`<div class="week-time">${pad(h)}</div>${days.map(d=>`<div class="week-cell">${eventsFor(d).filter(e=>(e.time||'').startsWith(pad(h))).map(e=>`<div class="mini-event">${escapeHtml(e.time||'')} ${escapeHtml(e.title)}</div>`).join('')}</div>`).join('')}`).join('')}</div></div>`;
}
function renderMonth(){
  ...
  for(let day=1;day<=daysIn;day++){const d=new Date(y,m,day), items=eventsFor(d); cells.push(`<div class="month-cell"><div class="month-day-number">${day}</div>${items.slice(0,3).map(e=>`<div class="mini-event">${escapeHtml(e.time||'')} ${escapeHtml(e.title)}</div>`).join('')}</div>`)}
...
function renderUpcoming(){
  ...
  const list=[...state.events].filter(e=>e.date>=nowKey && (!e.done || state.filter==='all')).sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time)).slice(0,15);
  $('#upcomingList').innerHTML=list.length?list.map(e=>`<article class="agenda-card"><div class="time-col"><strong>${e.time||'—'}</strong><span>${fmtShort(e.date)}</span></div><div class="card-main"><h3>${escapeHtml(e.title)}</h3><p>${e.location?escapeHtml(e.location):'Sem local'}</p></div><button class="source-badge ${e.source}" onclick="toggleEvent(${e.id})">${state.showSource?sourceLabel[e.source]:(e.done?'✓':'○')}</button></article>`).join(''):'<div class="panel muted">Nenhum compromisso encontrado.</div>';
}
window.toggleEvent=id=>{const e=state.events.find(x=>x.id===id); if(e){e.done=!e.done;save();render()}}
function renderTasks(){ $('#taskList').innerHTML=state.tasks.map(t=>`<article class="task-card ${t.done?'done':''}"><button class="check ${t.done?'done':''}" onclick="toggleTask(${t.id})">${t.done?'✓':''}</button><div><div class="task-title">${escapeHtml(t.title)}</div><div class="task-meta">${fmtShort(t.date)}</div></div></article>`).join('') }
window.toggleTask=id=>{const t=state.tasks.find(x=>x.id===id); if(t){t.done=!t.done;save();render()}}
function renderReminders(){ $('#reminderList').innerHTML=state.reminders.length?state.reminders.map(r=>`<article class="reminder-card"><span>${escapeHtml(r.text)}</span><button class="remove-btn" onclick="removeReminder(${r.id})">Remover</button></article>`).join(''):'<div class="panel muted">Nenhum lembrete.</div>' }
...
$('#prevDate').onclick=()=>{state.date=addDays(state.date,state.calendarView==='month'?new Date(state.date.getFullYear(),state.date.getMonth()+1,0).getDate():state.calendarView==='week'?-7:-1);render()};
$('#nextDate').onclick=()=>{state.date=addDays(state.date,state.calendarView==='month'?new Date(state.date.getFullYear(),state.date.getMonth()+1,0).getDate():state.calendarView==='week'?7:1);render()};
$('#todayBtn').onclick=()=>{state.date=new Date();render()};
$('#clearCompleted').onclick=()=>{state.events=state.events.filter(e=>!e.done);save();render()};
$('#newEventBtn').onclick=()=>{ $('#eventDate').value=isoDate(state.date); $('#eventDialog').showModal() };
...
$('#eventForm').addEventListener('submit',e=>{
  ...
  save();e.target.reset();$('#eventDialog').close();render();
});
$('#addReminderBtn').onclick=()=>{const v=$('#quickReminder').value.trim();if(!v)return;state.reminders.push({id:Date.now(),text:v});$('#quickReminder').value='';save();render()};
